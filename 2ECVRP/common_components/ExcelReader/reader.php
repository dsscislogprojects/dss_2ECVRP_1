...
//end of changes by mmp
    ...
    // DUMP AN HTML TABLE OF THE ENTIRE XLS DATA
    // =========================================
    function dump($row_numbers=false,$col_letters=false,$sheet=1,$table_class='excel') {

        for($row=1;$row<=$this->rowcount($sheet);$row++) {

            $val = $this->val($row,1,1);

            echo $this->val($row,1,1);
            ...
            if($val == $equip){
                ...
                for($col=1;$col<=$this->colcount($sheet);$col++) {
                    ...
                    if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]) {
